    ...
    return (
        ...
        >
            ...
            {payload.map((entry, idx) => (
                <div key={idx}>{entry.name ? `${entry.name}: ${entry.value}` : entry.value}</div>
            ))}
        </div>
    );
};
...
export default function AnalyticsPage() {
    ...
    return (
        <div
            className="
        relative z-[1]
        pt-28 md:pt-32 scroll-mt-32
        min-h-screen flex flex-col items-center
        py-12 px-6 bg-transparent
        overflow-hidden
      "
        >
            {/* Header */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full max-w-6xl">
                ...
                {/* Participants */}
                <div className="bg-transparent border border-amber-500/40 rounded-2xl shadow-[0_0_20px_rgba(250,204,21,0.25)] p-5 backdrop-blur-sm hover:shadow-[0_0_25px_rgba(251,146,60,0.35)] transition">
                    <h2 className="text-center text-orange-300 text-lg font-semibold mb-3">
                    ...
                    <ResponsiveContainer width="100%" height={350}>
                        <BarChart data={participantsData}>
                            ...
                            <Tooltip content={<CustomTooltip />} />
                            <Legend wrapperStyle={{ color: "#fdba74" }} />
                            <Bar dataKey="participants" fill="url(#warmGradient)" radius={[8, 8, 0, 0]} />
                            <defs>
                                <linearGradient id="warmGradient" x1="0" y1="0" x2="0" y2="1">
                                    <stop offset="0%" stopColor="#f87171" />
                                    <stop offset="100%" stopColor="#facc15" />
